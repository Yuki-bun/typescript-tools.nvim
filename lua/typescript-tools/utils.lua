...
function M.check_minimum_nvim_version()
  ...
  local is_compatible = vim.version.cmp(current_version, required_version) > -1
  local version_str =
    string.format("%d.%d.%d", current_version.major, current_version.minor, current_version.patch)

  if is_compatible then
    return true, string.format("Neovim %s (>= 0.11.2 required)", version_str)
  else
    return false, string.format("Neovim %s found, but >= 0.11.2 is required", version_str)
  end
end
